$(function(){
    $('.top-bar').eq(0).remove();
    if(location.href.indexOf('userid')>0){
        getUserInfo();
    }
    ...
    $('#password').keydown(function(){
        ...
        if(!pattern1.test(pw)||!pattern2.test(pw)||!pattern3.test(pw)||pw.length<8||pw.length>50) {
            $('#msg-password-prev').html('영문,숫자,특수기호 8자리 이상으로 구성하여야 합니다.');
        }else{
            $('#msg-password-prev').html('');
        }
...
function getUserInfo(){
     $.ajax({
         ...
         success: function(res){
             console.log(res)
             if(res || res.confirm_yn == 'W'){
                 $('#step0.on').removeClass('on')
                 $('#step2').addClass('on')
                 $('#step2 .info').show()
                 ...
                 _.each(res.images_ids, function(v, i){
                     $.ajax({
                         url: _host + '/func/getImagePath',
                         data: {id : v},
                         success: function (img){
                             let objImage = new Image();   // Create new img element
                            if(img.indexOf('joinBiz')>0){
                                objImage.src = _display+img;
                                objImage.onload = function(e){
                                    draw(objImage,'canvas1', 'dispImg1')
                                };
                            }else if(img.indexOf('joinMarket')>0){
                                objImage.src = _display+img;
                                objImage.onload = function(e){
                                    draw(objImage,'canvas2', 'dispImg2')
                                };
                            }else if(img.indexOf('joinCard')>0){
                                objImage.src = _display+img;
                                objImage.onload = function(e){
                                    draw(objImage,'canvas3', 'dispImg3')
                                };
                            }
                         }
                     })
...
function goStep4(){
    ...
    }else if($('#password').val() == '' ||  $('#msg-password').html() != '' ||  $('#msg-password-prev').html() != ''){
        modal.alert('비밀번호를 확인하세요.')
    }else{
        let prms = [];
        if($('#hasImages').val() == 'Y'){
            let p = null;
            if($('#dispImg1').attr('src') != null){
                p = new Promise(function(resolve, reject){
                    uploadImage($('#dispImg1').attr('src'), 'joinBiz', resolve)
                })
                prms.push(p)
            }
            if($('#dispImg2').attr('src') != null){
                p = new Promise(function(resolve, reject){
                    uploadImage($('#dispImg2').attr('src'), 'joinMarket', resolve)
                })
                prms.push(p)
            }
            if($('#dispImg3').attr('src') != null){
                p = new Promise(function(resolve, reject){
                    uploadImage($('#dispImg3').attr('src'), 'joinCard', resolve)
                })
                prms.push(p)
            }
        }
    ...
    $file.change(function(){
        ...
        reader.onload = function(ev) {
            ...
            img.onload = function(e){
                draw(img, canvas_id, img_id)
            }
        }
    })
}

function draw(img, canvas_id, img_id){
